<script lang="ts">
    ...
    const navigation = [
        { name: 'Overview', href: '/', inert: true},
        { name: 'Briefing', href: '/hub/missions', inert: true},
    ]
    let open = false;
</script>
...
<nav class="bg-gray-900 rounded-lg">
            ...
            <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
                <button type="button" on:click="{() => open = !open}"class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
                    <svg class="block h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{open ? 'M4 6h16M4 12h16M4 18h16' : 'M6 18L18 6M6 6l12 12'}" />
                    </svg>
                </button>
            </div>
                ...
                <div class="hidden sm:block sm:ml-6">
                    <div class="flex space-x-4">
                        {#each navigation as item}
                            <Link href={item.href} class="{$page.url == item.href ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'} px-3 py-2 rounded-md text-md font-bold">{item.name}</Link>
                        {/each}
                    </div>
                </div>
            </div>
