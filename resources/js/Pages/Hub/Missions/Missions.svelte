...
<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
    ...
    <h3 class="my-2 text-center text-xl font-bold uppercase leading-tight text-blue-600">Unplayed missions</h3>
    <div
        class="col-span-full grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6"
    >
        {#each missions as mission}
            {#if !mission.last_played && (mission.verified_by || can.test_missions)}
                <Card {mission} />
            {/if}
        {/each}
    </div>
    <h3 class="my-2 text-center text-xl font-bold uppercase leading-tight text-blue-600">Played missions</h3>
    <div
        class="col-span-full grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6"
    >
        {#each missions as mission}
