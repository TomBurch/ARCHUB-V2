...
<nav class="bg-gray-900 rounded-lg">
    ...
    <div class='sm:hidden' id="mobile-menu">
        <div class="{open ? '' : 'hidden'} px-2 pt-2 pb-3 space-y-1">
            {#each navigation as item}
                <button on:click="{() => selected = item}" class="{selected.name == item.name ? 'border-b-4 border-b-indigo-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'} block px-3 py-2 rounded-t-md text-md font-medium w-full text-left">{item.name}</button>
            {/each}
        </div>
    </div>
</nav>
